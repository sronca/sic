...
angular.module('ptof.controlles.gestione.sezione_11_progettiCV', [
  'ptof.commons.services',                                            
  'ptof.directives',
  'base64',
  'ptof.controlles.gestione'
])
.controller('GestioneSezione-11-GestioneProgettiCVCtrl', function ($scope, $rootScope, $window , $location , $base64, $timeout ,gestioneDocServicesFactory , commonsUtilityPtofServicesFactory , ptofCommonsServicesFactory) {
...
//	    	   // CALCOLO I TIPO PROGETTI IN FUNZIONE DELL'AMBITO
//	   		   ptofCommonsServicesFactory.getTipologia({tipologica :{tipo:'TIPOPROGETTI' , itemToFilter: $scope.vm.tabella1.newItem.ambito }})
//					.success(function (data, status, config, headers) {
//						$scope.vm.attribForm.tipologiaProgettoL = data.attribForm.TIPOPROGETTI;	
//			   })
			...
			controlliSuperati : function (key){
				...
				var fieldErrors =[];
			 	...
			 	commonsUtilityPtofServicesFactory.testCombo( { fieldErrors:fieldErrors,o:'o' , field :'item1_tipologiaProgetto', value : this.newItem.tipologiaProgetto});
	...
	var init  = function () {
		...
		.success(function (data, status, config, headers) {
			...
			$.each( $scope.modelGestioneSezione.sezione.componenti , function (index, componente){
				...
				if (   componente.tipoComponente == 'S_PROGETTI_CURRICULARI'    ){
