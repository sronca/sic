...
angular.module('ptof.controlles.gestione.sezione_19_collaborazioni_entilocali', [
  'ptof.commons.services',                                            
  'ptof.directives',
  'base64',
  'ptof.controlles.gestione'
])
.controller('GestioneSezione-19-GestioneCollEntiLocaliCtrl', function ($scope, $rootScope, $window , $location , $base64, gestioneDocServicesFactory , commonsUtilityPtofServicesFactory , ptofCommonsServicesFactory) {
	console.log('GestioneSezione-19');
	...
	$scope.vm = {
		...
		tabella1 : {
			...
			titolo : '',
			tabella : { 
				cols : [ // entiLocali, tipologiaPromozione, azioniIntraprese, areaTematicaPNSD   
				        { title:'Enti Locali',data:'entiLocali',cl:''},
				        { title:'Tipologia Promozione',data:'tipologiaPromozione',cl:''},
				        { title:'',data:'key',cl:''} ],
				items : []
			},
	     ...
	     tabella2 : {
				...
				titolo : '',
				tabella : { 
					cols : [   // key, convenzioni, organizzazione, note, areaTematicaPNSD
					        { title:'Convenzioni',data:'convenzioni',cl:''},
					        { title:'Organizzazione',data:'organizzazione',cl:''},
					        { title:'',data:'key',cl:''} ],
					items : []
				},
				loadDialog : function (key){
				...
				controlliSuperati : function (key){
					...
					var fieldErrors =[];
			 		...
			 		commonsUtilityPtofServicesFactory.testTextAreas( { fieldErrors:fieldErrors, o:'o', perFix :'_t2',
						 that : this.newItem ,
						 field : ['convenzioni', 
						          'organizzazione']});
	...
	var init  = function () {
		...
		.success(function (data, status, config, headers) {
			...
			$.each( $scope.modelGestioneSezione.sezione.componenti , function (index, componente){
				...
				if (  'S_PROMOZIONE_RAPPORTI_ENTI_LOCALI_TERRITORIO'==componente.tipoComponente  ){
				...
				if (  'S_CONVENZIONI_STIPULATE_POTENZIAMENTO_FORMATIVO'==componente.tipoComponente  ){
